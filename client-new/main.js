...
if (!gotTheLock) {
    ...
    app.whenReady().then(async () => {
        try {
            ...
            const unpackedPath = app.isPackaged ? appPath.replace('app.asar', 'app.asar.unpacked') : appPath;
            
            const documentRoot = path.join(unpackedPath, 'public');
            console.log('Document Root:', documentRoot);

            // Démarrer le serveur PHP
            phpServer = new PHPServer(documentRoot);
